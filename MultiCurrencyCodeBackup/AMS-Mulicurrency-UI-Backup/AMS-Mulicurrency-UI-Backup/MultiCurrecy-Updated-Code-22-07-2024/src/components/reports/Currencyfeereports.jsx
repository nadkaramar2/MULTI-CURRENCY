...
export default function Currencyfeereports() {
  ...
  const gltype = async () => {
    try {
      const response = await amsApi.post(
        `multiCurrencyFeeType_Master/getAccountType`,
        {},
        {
          headers: {
      ...
      if (response.data.code === "S0000") {
        setTypedata(response.data.multiCurrencyFeeTypeMasters);
      } else {
  ...
  const FeesDeatils = async () => {
    if (accounttype === "" || accounttype.length === 0) {
      setError("Please select Account Type");
    } else {
      try {
        const response = await amsApi.post(
          `multiCurrencyFeeType_Master/ViewFeeType`,
    ...
    <AppLayout>
      <div className="max-w-full mx-auto h-full scale-96">
        <div className="w-full shadow-md mt-2">
          <div className="px-4  sm:px-12  bg-blue-200">
            <div className=" flex  items-center justify-between">
              <p className="text-base  sm:text-xm text-black :text-2xl  leading-normal ">
                Multi Currency Fee Reports
              </p>
            </div>
          </div>
        </div>
