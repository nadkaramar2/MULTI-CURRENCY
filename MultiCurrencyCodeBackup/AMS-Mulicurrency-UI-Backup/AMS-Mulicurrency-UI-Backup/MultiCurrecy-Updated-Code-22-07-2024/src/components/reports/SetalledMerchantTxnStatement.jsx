...
export default function SetalledMerchantTxnStatement() {
  ...
  const filteredData = alldata.filter((data) => {
    ...
    return (
      (strGLAccountType &&
        strGLAccountType.toLowerCase().includes(lowerCasedSearchKeyword)) ||
      (strGLAccountDescription &&
        strGLAccountDescription
          .toLowerCase()
      ...
      (strOpeningBalance &&
        strOpeningBalance.toLowerCase().includes(lowerCasedSearchKeyword)) ||
      (strClosingBalance &&
    ...
    if (
      ...
      try {
        const response = await amsApi.post(
          `merchantTxnMaster/NGN/viewSettledMerchantTxns`,
          {
            // cid: cid,
        ...
        if (response.data.code === "S0000") {
          setAlldata(response.data.merchantTransactionSettledList);
          ...
          <div className=" sm: px-11 rounded-t-lg ">
            <div className=" flex  items-center justify-between">
              <p className="text-base  sm:text-xm text-black :text-2xm  leading-normal ">
                View Setallet Merchant Txn Statement
              </p>
              <div>
                      ...
                      {filteredData.map(
                        ({
                          ...
                          >
                            ...
                            <td className="  px-11 py-2 text-right whitespace-nowrap ">
                              <div className="text-xs  font-medium text-gray-900 ">
                                {highlightKeyword(cid || "-", searchKeyword)}
                              </div>
                            </td>
                          </tr>
                        )
