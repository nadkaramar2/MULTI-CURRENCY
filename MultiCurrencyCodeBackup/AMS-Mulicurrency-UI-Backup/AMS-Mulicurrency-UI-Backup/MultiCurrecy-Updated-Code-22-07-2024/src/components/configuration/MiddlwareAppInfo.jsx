...
export default function MiddlwareAppInfo() {
  const [strAppId, setstrAppId] = useState("");
  const [strChannelCode, setstrChannelCode] = useState("");
  const [strInitialValue, setstrInitialValue] = useState("");
  ...
  const handleSubmit = async (event) => {
    ...
    if (strAppId === "" || strAppId.length === 0 || strChannelCode === "") {
      setError(true);
    } else {
      try {
        const response = await amsApi.post(`middleware-app-info/add`, {
          strParticipantId: participantID,
          strAppId: strAppId,
          strChannelCode: strChannelCode,
          strInitialValue: strInitialValue,
          strUser: userId,
        });
              ...
              <div className=" px-4  sm:p-2 bg-white  ">
                <div className="grid gap-2 px-4  md:grid-cols-1 ">
                  <div className="flex">
                    ...
                    >
                      App Id<span className="text-red-600 px-2">*</span>
                    </label>
                    <input
                      type="text"
                      id="strAppId"
                      value={strAppId}
                      onChange={(e) => setstrAppId(e.target.value)}
                  ...
                  <div className="flex">
                    ...
                    <input
                      ...
                      id="strChannelCode"
                      disabled={!strAppId}
                      value={strChannelCode}
                      onChange={(e) => setstrChannelCode(e.target.value)}
                      className="block w-60 px-3 py-1 text-xs font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-500 rounded transition ease-in-out m-0"
                      placeholder="Enter Channel Code"
                    />
                    {error && strChannelCode.length <= 0 ? (
                      <p className="text-red-500   text-xs font-medium">
                        Please Ente Channel Code
                      </p>
                    ) : (
                      ""
                    )}
                  ...
                  <div className="flex">
                    <label
                      htmlFor="text"
                      className="block w-auto px-3 py-1 mr-6 text-base font-normal text-gray-700 bg-white  "
                      // className="block text-xs font-semibold text-gray-700"
                    >
                      Initial Value
                    ...
                    <input
                      type="text"
                      id="strInitialValue"
                      disabled={!strChannelCode}
                      value={strInitialValue}
                      onChange={(e) => setstrInitialValue(e.target.value)}
                      className="block w-60 px-3 py-1 text-xs font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-500 rounded transition ease-in-out m-0"
                      placeholder="Enter Initial Value"
                    />
                    {error && strInitialValue.length <= 0 ? (
                      <p className="text-red-500 text-xs font-medium">
                        Please Ente Initial Value
                      </p>
                    ) : (
                      ""
                    )}
