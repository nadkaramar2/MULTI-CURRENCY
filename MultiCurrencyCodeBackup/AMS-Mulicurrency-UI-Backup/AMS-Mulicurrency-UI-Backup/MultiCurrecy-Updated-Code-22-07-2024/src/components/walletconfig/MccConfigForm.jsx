...
export default function MccConfigForm() {
    ...
    if (accounttype === "" || accounttype.length === 0) {
      ...
      try {
        const response = await amsApi.post(
          `blocked_mcc_account_type_wise/getBlockMccAcountTypeWise`,
          {
        ...
        if (response.data.code === "S0000") {
          setBlockdata(response.data.accountTypeWiseBlockedMccMasters);
          setCheckpoint(1);
        } else {
  ...
  const Selected = async () => {
    if (accounttype === "" || accounttype.length === 0) {
      ...
      try {
        ...
        if (response.data.code === "S0000") {
          setSelectdata(response.data.mccParticipantListData);
  ...
  const handleSubmit = async (selectedRows) => {
    ...
    if (selectedMccCodes === "" || selectedMccCodes.length === 0) {
      ...
      try {
        const response = await amsApi.post(
          `blocked_mcc_account_type_wise/addMccCode`,
          {
            strAccounType: accounttype,
  ...
  const sortedData = [...blockdata].sort((a, b) => {
    if (sortOrder === "asc") {
  ...
  // Sort the data based on the chosen column and order
  const sortedData1 = [...selectdata].sort((a, b) => {
    if (sortOrder1 === "asc") {
    ...
    <AppLayout>
      <div className="max-w-full mx-auto h-full">
            ...
            <div className="overflow-hidden shadow sm:rounded-md">
              <div className=" px-4 sm:p-2 bg-white ">
                <div className="grid gap-6  md:grid-cols-3 px-8">
                  ...
                  <div className="flex justify-between items-end px-4 py-1 text-right sm:px-2 ">
                    <button
        ...
        <div className="flex justify-between items-center sm:px-6 lg:px-8 bg-blue-200    mt-2 rounded-t-lg ">
          ...
          <div className="flex justify-center rounded-md border border-transparent px-2 mx-2 text-xs font-xs text-white ">
                  ...
                  {sortedData.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan="6" className="px-6 py-2.5 text-center">
                        <span className="text-sm font-mormal text-gray-500">
                          No data available.
                        </span>
                      </td>
                    </tr>
        ...
        <div className="flex justify-between items-center sm:px-6 lg:px-8 bg-blue-200 rounded-t-lg  mt-2">
          <p className="text-base  sm:text-xm text-black :text-2xl  leading-normal ">
            BLOCKED MCC ACCOUNT TYPE WISE
          </p>
          <div className=" flex justify-center px-2 mx-2 text-xs ">
            <div className="pt-2 relative mx-auto text-gray-600">
              <input
                title="Search Data"
                type="search"
