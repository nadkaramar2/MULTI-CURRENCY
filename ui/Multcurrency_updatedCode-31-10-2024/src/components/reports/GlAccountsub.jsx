...
export default function GlAccountsub() {
  ...
  const { strGLAccountType } = useParams();
  const [GLAccountType, setStrGLAccountType] = useState(strGLAccountType);
  const [searchQuery, setSearchQuery] = useState("");
  const [itemPage, setItemPage] = useState(20);
  const [tcount, setTcount] = useState(1);
  const [error1, setError1] = useState("");
  const { strAccountNumber } = useParams();
  const [AccountNumber, setAccountNumber] = useState(strAccountNumber);
    ...
    if (
      ...
      try {
        ...
        if (response.data.code === "S0000") {
          setData(response.data.gLAccountStatementList);
    ...
    const Search = async () => {
      if (startdate === "" || enddate === "") {
        ...
        try {
          ...
          if (response.data.code === "S0000") {
            setData(response.data.glAccountSearchList);
            setChecked(false);
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto h-full">
        <div className="w-full shadow-md mt-2 ">
          <div className="text-6 sm:px-10 bg-blue-800">
            <div className=" flex  items-center justify-between">
              <p className="flex justify-start  text-white text-md  font-semibold leading-normal">
                GL Account Statements
              </p>
              <div>
                <button className="inline-flex sm:ml-3   sm:mt-0 items-start justify-start text-6 focus:outline-none rounded text-xs font-medium">
                  <Switch
                    checked={checked}
          ...
          <div className=" md:col-span-2 lg:col-span-1 ">
            <form className="">
              <div className="overflow-hidden shadow ">
                <div className="  sm:p-2 bg-white ">
                  <div className="grid gap-6   md:grid-cols-3 lg:grid-cols-3 px-8 ">
                    <div>
                      <label
                        htmlFor="text"
            ...
            <div className="grid gap-6 mb-6 md:grid-cols-3 "> */}
                    {dateOpen && (
                      <>
                        {/* <div className="grid gap-6 mb-2 mt-2 md:grid-cols-2 lg:grid-cols-3 px-8"> */}
                        <div>
                          <label
                    ...
                    <div className="">
                      <button
                        ...
                        className="inline-flex justify-center rounded-md border border-transparent bg-blue-500 py- px-5 mx-2 my-5 text-xs font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                      >
                      ...
                      <button
                        ...
                        type="button"
                        onClick={handleClick}
                        className="inline-flex justify-center rounded-md border border-transparent bg-rose-500 py- px-5 mx-2 my-5 text-xs font-medium text-white shadow-sm hover:bg-rose-700 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2"
                      >
                        <XMarkIcon className="h-3 w-3" />
                      </button>

                      <button
                        title="Clcik Here to download Button"
                        onClick={() => openModal()}
                        className="inline-flex justify-center rounded-md border border-transparent bg-green-500 py- px-5 mx-2 my-5 text-xs font-medium text-white shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                      >
            ...
            <div ref={conponentPDF} style={{ width: "100%" }}>
              <table className="w-full text-xs  text-left text-black dark:text-blue-100">
                <thead className="w-full text-white border-b sticky top-0 bg-blue-500 ">
                  <th
                    scope="col"
                    className="py-1 text-6 whitespace-nowrap"
                ...
                <tbody>
                  {sortedData.length > 0 ? (
                    <>
                      {sortedData.map(
                        ({
                          ...
                          >
                            <td className="text-6 py-1 whitespace-nowrap">
                            ...
                            <td className="text-6 py-1 text-end whitespace-nowrap">
                              <div className="text-xs font-medium text-gray-900">
                                {strAmount || ""}
                              </div>
                            </td>
                    ...
                    <tr>
                      <td colSpan="6" className="text-6 py-1.5 text-center">
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className=" text-6 py-3 sm:text-6 sm:flex sm:flex-row-reverse">
                    <button
                      title="Clcik Here to Download Pdf"
                      type="button"
                      className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm text-6 py- bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-xs"
                      onClick={generatePDF}
                    >
                      PDF
                    </button>
                    <button
                      ...
                      type="button"
                      className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm text-6 py- bg-orange-600 text-base font-medium text-white hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-xs"
                    ...
                    <button
                      title="Cancel Here"
                      type="button"
                      className="mt-1 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm text-6 py-1 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-xs"
                      onClick={closeModal}
                    >
                      Cancel
                    </button>
