...
export default function ViewRewardType() {
...
const [tcount, setTcount] = useState(1);
  ...
  const viewChannel = async () => {
    try {
      const response = await amsApi.post(
        `rewardPoints/viewTypeCreation/pagination/${itemPage}/${tcount}`,
        // `rewardPoints/NGN/viewTypeCreation/pagination/${itemPage}/${tcount}`,
        {}
      );
      if (response.data.code === "S0000") {
        setAlldata(response.data.rewardPaginationList);
  ...
  const Search = async () => {
      try {
        const response = await amsApi.post(
          `rewardPoints/viewRewardPointSearch/pagination/20/1`,
          {
            keyword: searchQuery,
          }
        );
        if (response.data.code === "S0000") {
          setAlldata(response.data.rewardPointSearchList);
        } else {
          handleShowError(response.data.message);
        }
      } catch (error) {
            ...
            <div className="table-wrp block max-h-[33rem] ">
              <table className="w-full text-xs text-left text-clack dark:text-blue-100">
                ...
                <tbody>
                  <>
                    {alldata.map(
                      ({ pointsType, pointsDescr, createdDate, createdBy }) => (
                        <tr
                          key={uuidv4()}
