...
const MultiLoadMoneyReport = () => {
    ...
    if (
      ...
      try {
        const response = await amsApi.post(
          `account-statement/getloadmoneyreport`,
          {
        ...
        if (response.data.code === "S0000") {
          ...
          setalldata(response.data.loadMoneyTxnDetailsList);
  ...
  const Search = async () => {
    try {
      const response = await amsApi.post(
        `account-statement/loadmoneyreportSearch/pagination/20/1`,
        {
          keyword: searchQuery,
        }
      );
      if (response.data.code === "S0000") {
        setalldata(response.data.loadMoneyTxnSearch);
        setChecked(false);
  ...
  const currencyCode = async () => {
    try {
      ...
      if (response.data.code === "S0000") {
        setchanneldata(response.data.currencymasterdata);
      } else {
        // showError(response.data.message);
      }
    } catch (error) {
          ...
          <div className="  sm:px-4 bg-blue-200 ">
            <div className=" flex  items-center justify-between">
              <p className="flex justify-start font-sans sm:text-xm text-black text-sm leading-normal">
                MultiCurrency LoadMoney Reports
              </p>
            ...
            <div ref={conponentPDF} style={{ width: "100%" }}>
              <table className="w-full text-xs text-left text-black dark:text-blue-100">
                <thead className="text-xm border-b sticky top-0 text-gray-400 uppercase bg-gray-100 dark:text-white">
                  ...
                  >
                    currency Code {renderSortArrow("currencyCode")}
                  </th>
                  <th
                    scope="col"
                    className="py-3 px-2 whitespace-nowrap"
                  ...
                  <th
                    scope="col"
                    className="py-3 px-2 whitespace-nowrap"
                    onClick={() => handleSort("balancedLRS")}
                  >
                    Balance LRS {renderSortArrow("balancedLRS")}
                  </th>

                  <th
                    scope="col"
                    className="py-3 px-2 whitespace-nowrap"
                    onClick={() => handleSort("fyBalance")}
                  >
                    Fy Balance {renderSortArrow("fyBalance")}
                  </th>
                  <th
                    scope="col"
                    className="py-3 px-2 whitespace-nowrap "
                  ...
                  <th
                    scope="col"
                    className="py-3 px-2 whitespace-nowrap text-right"
                    onClick={() => handleSort("tcsCalculatedAmt")}
                  >
                    tcs Calculated Amt {renderSortArrow("tcsCalculatedAmt")}
                  </th>

                  <th
                    scope="col"
                    className="py-3 px-2 whitespace-nowrap text-right"
                    onClick={() => handleSort("tcsOn")}
                  >
                    tcs On {renderSortArrow("tcsOn")}
                  </th>
                  <th
                    scope="col"
                    className="py-3 px-2 whitespace-nowrap text-right"
...
export default MultiLoadMoneyReport;
