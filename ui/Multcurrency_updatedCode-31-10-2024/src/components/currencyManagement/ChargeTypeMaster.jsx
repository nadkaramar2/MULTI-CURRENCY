...
export default function ChargeTypeMaster() {
  ...
  const feetypeSubmit = async (event) => {
    ...
    if (
      ...
      try {
        const response = await amsApi.post(`chargetype/addchargetype`, {
  ...
  const ViewNetwork = async () => {
    try {
      ...
      if (response.data.code === "S0000") {
        setResData(response.data.chargeType);
      } else {
              ...
              <div className="px-4 py-2 sm:p-2 bg-white">
                <div className="grid gap-2  lg:grid-cols-1 md:grid-cols-1 px-8">
                  ...
                  <div className="flex">
                    <label
                      htmlFor="text"
                      className="block w-auto px-3  py-1 mr-7  text-base font-normal text-gray-700 bg-white  "
                      // className="block text-xs font-semibold text-gray-700"
                    >
                      Charge Amount <span className="text-red-600 px-1">*</span>
                    </label>
                    <input
                      type="text"
                      id="chargeAmt"
                      name="chargeAmt"
                      value={chargeAmt}
                      disabled={!chargeDescription}
                      onChange={(e) => setchargeAmt(e.target.value)}
                      className="block w-60 px-3 py-1 sm:text-xs font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-500 rounded transition ease-in-out m-0"
                      placeholder="Enter Charge  Amount"
                      autoComplete="off"
                    />
                    {error && chargeAmt.length <= 0 ? (
                      <p className="text-red-500   text-xs font-medium">
